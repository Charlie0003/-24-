...
class Main:
    def __init__(self):
        ...
        for i in range(199):
            a, b = self.run(shop_num, shop_front, exam_num, shop_min, shop_max, exam_kind)
            input_values.append(a)
            successful_times.append(b)
            shop_front += 0.5
        self.draw_table(input_values, successful_times, shop_min, shop_max, shop_num, exam_kind, exam_num)
    ...
    def run(self, shop_num, shop_front, exam_num, shop_min, shop_max, exam_kind):
        ...
        input_values.append(shop_front)
        # 顺便将值也打印下
        print(shop_front)
        print(a)
        print()
        # 制作图表
        return input_values, successful_times
    ...
    def draw_table(self, input_values, successful_times, shop_min, shop_max, shop_num, exam_kind, exam_num):
        """绘制图表"""
        fig, ax = plt.subplots()
        ax.plot(input_values, successful_times, linewidth=3)
        ax.set_title(f"{shop_num} {shop_min}-{shop_max} uniform", fontsize=24)
        # 保存至桌面
        plt.savefig(f"E:\\table{shop_num} {exam_num}.png")
        # 展示下
        plt.show()
